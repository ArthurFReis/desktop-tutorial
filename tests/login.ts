...
export async function pageResponsivoLogin(page: Page,) {
    ...
    for(const tamanho in tamanhos.width){
      ...
      await page.screenshot({path: `Evidencias/login/Responsivo/Saucedemo/mobile-${tamanhos.nomes[tamanho]}.png`});
    }
    else {
      await page.setViewportSize({ width: tamanhos.width[tamanho], height: tamanhos.height[tamanho]});
      await page.screenshot({path: `Evidencias/login/Responsivo/Inventory/mobile-${tamanhos.nomes[tamanho]}.png`});
    }
